...
use esp_idf_svc::sys::{
    ...
    esp_netif_get_handle_from_ifkey, esp_netif_dhcps_stop, esp_netif_dhcps_start,
    esp_netif_dhcps_option, esp_netif_set_dns_info, esp_netif_dns_info_t,
...
unsafe fn force_dns_dhcp(ip_addr: u32) {
    let ap = esp_netif_get_handle_from_ifkey(b"WIFI_AP_DEF\0".as_ptr() as *const _);
    if ap.is_null() {
    ...
    D::Error: core::fmt::Debug,
{
    ...
    let config = AccessPointConfiguration {
        ssid: "FZ-Module".try_into().unwrap(),
        password: "motdepasse123".try_into().unwrap(),
    ...
    unsafe { force_dns_dhcp(ip_u32); }

    // Serveur DNS captif
    std::thread::Builder::new()
        .stack_size(4096)
        .spawn(|| crate::captive_dns::run([192, 168, 71, 1]))?;
    ...
    for path in ["/generate_204", "/gen_204", "/hotspot-detect.html", "/ncsi.txt", "/connecttest.txt", "/redirect", "/canonical.html", "/success.txt"] {
    ...
    // Boucle principale - Affichage OLED
    loop {
        ...
        let nb = (creds.len() as usize).min(4);
        let mut y = 104i32;
