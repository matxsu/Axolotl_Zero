...
//!
//! WIP capture/replay : les fonctions polling (`monitor_rssi`, `capture_pulses`,
//! `replay_pulses`) ne sont pas encore câblées dans l'UI — la couche capture
//! fidèle via RMT (GDO0=GPIO38) arrive dans un commit suivant et remplacera le
//! polling. `allow(dead_code)` temporaire le temps de la construction.
#![allow(dead_code)]
...
impl<'d> Cc1101<'d> {
    ...
    fn write_reg(&mut self, addr: u8, val: u8) -> anyhow::Result<()> {
        ...
        self.spi
            .write(&buf)
            .map_err(|e| anyhow::anyhow!("cc1101 write: {:?}", e))
    }
    ...
    fn read_reg(&mut self, addr: u8) -> anyhow::Result<u8> {
        ...
        self.spi
            .transfer(&mut rx, &tx)
            .map_err(|e| anyhow::anyhow!("cc1101 read: {:?}", e))?;
        Ok(rx[1])
    }
    ...
    fn read_status_reg(&mut self, addr: u8) -> anyhow::Result<u8> {
        ...
        self.spi
            .transfer(&mut rx, &tx)
            .map_err(|e| anyhow::anyhow!("cc1101 status: {:?}", e))?;
        Ok(rx[1])
    }
    ...
    fn strobe(&mut self, cmd: u8) -> anyhow::Result<u8> {
        ...
        self.spi
            .transfer(&mut rx, &tx)
            .map_err(|e| anyhow::anyhow!("cc1101 strobe: {:?}", e))?;
        Ok(rx[0])
    }
    ...
    fn burst_write(&mut self, addr_byte: u8, data: &[u8]) -> anyhow::Result<()> {
        ...
        self.spi
            .write(&buf)
            .map_err(|e| anyhow::anyhow!("cc1101 burst_write: {:?}", e))
    }

    fn burst_read(&mut self, addr_byte: u8, out: &mut [u8]) -> anyhow::Result<()> {
        let tx_len = 1 + out.len();
        let tx: Vec<u8> = std::iter::once(addr_byte)
            .chain(std::iter::repeat_n(0x00, out.len()))
            .collect();
        let mut rx = vec![0u8; tx_len];
        self.spi
            .transfer(&mut rx, &tx)
            .map_err(|e| anyhow::anyhow!("cc1101 burst_read: {:?}", e))?;
    ...
    pub fn check_present(&mut self) -> bool {
        ...
        log::info!(
            "CC1101 PARTNUM={:#02x} VERSION={:#02x} IOCFG0={:#02x}",
            pn,
            ver,
            io0
        );
    ...
    pub fn configure_ook(&mut self, band: Band) -> anyhow::Result<()> {
        ...
        self.band = band;
        log::info!(
            "CC1101 OOK configure {} (FREQ={:02X}{:02X}{:02X})",
            band.name(),
            f2,
            f1,
            f0
    ...
    pub fn send_princeton(&mut self, bits: &[u8], te_us: u32, repeat: u8) -> anyhow::Result<()> {
        ...
        for &bit in bits {
            // 1 byte = 1 bit Princeton : 4 bits haut = pattern OOK (1110 ou 1000).
            let nibble: u8 = if bit != 0 { 0xE } else { 0x8 };
            payload.push(nibble << 4);
        }
        ...
        for _ in 0..repeat {
            ...
            for _ in 0..500u32 {
                ...
                if state == 0x01 || state == 0x16 {
                    break;
                } // IDLE ou TXFIFO_UNDERFLOW
                FreeRtos::delay_ms(2);
            }
        }
    ...
    pub fn capture_pulses(
        &mut self,
        timeout_ms: u32,
        threshold_dbm: i16,
    ) -> anyhow::Result<Vec<Pulse>> {
        self.strobe(SIDLE)?;
        self.strobe(SFRX)?;
        self.strobe(SRX)?;
        FreeRtos::delay_ms(5);
    ...
    pub fn replay_pulses(&mut self, pulses: &[Pulse], te_us: u32) -> anyhow::Result<()> {
        ...
        let mut payload: Vec<u8> = Vec::new();
        for p in pulses {
            let n_bits = (p.dur_us / te_us.max(50)).clamp(1, 64) as usize;
